...
$result = $con->query("SELECT * FROM `clothes` WHERE `cat` = 'men' ORDER BY RAND(5);");
...
if (!empty($sort)) {
    $stmt = $con->prepare("SELECT * FROM `clothes` WHERE `cat` = 'men' AND `sort` = ?");
...
if (!empty($color)) {
    $stmt = $con->prepare("SELECT * FROM `clothes` WHERE `cat` = 'men' AND `color` = ?");
...
if (!empty($discount)) {
    $stmt = $con->prepare("SELECT * FROM `clothes` WHERE `cat` = 'men' AND `discount` IS NOT NULL;");
    $stmt->execute();
    $res = $stmt->get_result();
    $result = $res->fetch_all(MYSQLI_ASSOC);
}
...
<header>
        <nav class="navbar">
            ...
            <ul class="nav-menu">
                ...
                <li class="nav-item"><a href="men.php" class="nav-link" id="color-cate">Men</a></li>
    ...
    <div class="all-products">
        ...
        <section class="filter-products">
            <div class="filter-container">
                <form class="form-filter" action="men.php" method="GET">
