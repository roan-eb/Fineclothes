...
$result = $con->query("SELECT * FROM `clothes` WHERE `cat` = 'women' ORDER BY RAND(5);");
...
if (!empty($sort)) {
    $stmt = $con->prepare("SELECT * FROM `clothes` WHERE `cat` = 'women' AND `sort` = ?");
...
if (!empty($color)) {
    $stmt = $con->prepare("SELECT * FROM `clothes` WHERE `cat` = 'women' AND `color` = ?");
...
if (!empty($discount)) {
    $stmt = $con->prepare("SELECT * FROM `clothes` WHERE `cat` = 'women' AND `discount` IS NOT NULL;");
    $stmt->execute();
    $res = $stmt->get_result();
    $result = $res->fetch_all(MYSQLI_ASSOC);
}
...
<body>
    <header>
        <nav class="navbar">
            ...
            <ul class="nav-menu">
                <li class="nav-item"><a href="women.php" class="nav-link" id="color-cate">Women</a></li>
                <li class="nav-item"><a href="men.php" class="nav-link">Men</a></li>
                <li class="nav-item"><a href="kids.php" class="nav-link">Kids</a></li>
    ...
    <div class="all-products">
        ...
        <section class="filter-products">
            <div class="filter-container">
                <form class="form-filter" action="women.php" method="GET">
